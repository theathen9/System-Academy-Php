...
<body class="container-fluid p-0 overflow-x-hidden">
    ...
    <script>
        ...
        function setupAddress(prefix) {
            ...
            // 🚨 FIX: stop if elements don't exist
            ...
            function hideOtherInputs() {
                [otherDistrict, otherCommune, otherVillage].forEach(el => {
                    if (!el) return;
            ...
            function populate(select, items) {
                items.forEach(v => {
                    const clean = v.trim();
                    select.add(new Option(clean, clean));
                });
            }

            // ---------- INIT ----------
            province.innerHTML = "<option value=''>-- Province --</option>";
            safeKeys(addressData).forEach(p => province.add(new Option(p, p)));
            ...
            province.onchange = () => {
                ...
                const provinceData = addressData[province.value];
                if (!provinceData) return;

                populate(district, safeKeys(provinceData.districts));
                district.disabled = false;
            };
            ...
            district.onchange = () => {
                ...
                if (district.value === "other") {
                    ...
                    otherDistrict.name = prefix + "_district";
                    commune.disabled = false;
                    return;
                }

                const districtData = addressData[province.value]?.districts?.[district.value];
                if (!districtData) return;

                populate(commune, safeKeys(districtData.communes));
                commune.disabled = false;
            };
            ...
            commune.onchange = () => {
                ...
                if (commune.value === "other") {
                    otherCommune.style.display = "block";
                    otherCommune.name = prefix + "_commune";
                    village.disabled = false;
                    return;
                }
                ...
                if (Array.isArray(villages)) {
                    populate(village, villages);
                } else {
                    populate(village, safeKeys(villages));
                }
            ...
            village.onchange = () => {
                if (village.value === "other") {
                    ...
                    otherVillage.name = prefix + "_village";
                    village.style.display = "none";
                } else {
